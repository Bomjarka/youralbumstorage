                        ...
                        >
                            ...
                            <form class="mt-5" method="post" action="{{ route('addRole') }}">
                                ...
                                <div>
                                    ...
                                    <input required placeholder="{{ trans('admin-permissions.permission-name') }}" type="text"
                                           name="role_name"
                        ...
                        @foreach($permissions as $permission)
                            <tr>
                                <td class="text-left py-3 px-4"><input type="hidden" class="role_id" name="role_id"
                                                                       value="{{ $permission->id }}">{{ $permission->id }}</td>
                                <td class="text-left py-3 px-4">{{ $permission->name }}</td>
                                <td class="text-left py-3 px-4"><input id={{$permission->id}}  class="role_description"
                                                                       name="role_description"
                                ...
                                @if (\App\Helpers\RoleHelper::has_permission(\App\Models\Permission::EDIT_ROLE, Auth::user()->id))
                                    <td class="text-left py-3 px-4">
                                        <form method="post" action="{{ route('editRole') }}">
                                            @csrf
                                            @method('post')
                                            <input type="hidden" id="rd_input_{{$permission->id}}" name="newRoleDescription"
                                                   value="">
                                            <input type="hidden" id="rid_input_{{$permission->id}}" name="roleId"
                                                   value={{$permission->id}}>
                                            <button type="click" id="save_role_{{$permission->id}}"
